...
namespace QuanLyKtx.Services
{
    public class StudentService
    {
        ...
        public bool AddStudent(Student student, out string errorMessage)
        {
            ...
            string checkSql = "SELECT COUNT(1) FROM dbo.Students WHERE StudentCode = @StudentCode";
            var count = DatabaseHelper.ExecuteScalar(checkSql, new[] { new SqlParameter("@StudentCode", student.StudentCode.Trim()) });
            ...
            try
            {
                string insertStudentSql = @"
                    INSERT INTO dbo.Students (StudentCode, FullName, DateOfBirth, Gender, Phone, Email, Address, ClassName, Faculty, CreatedAt)
                    VALUES (@StudentCode, @FullName, @DateOfBirth, @Gender, @Phone, @Email, @Address, @ClassName, @Faculty, GETDATE());
                    SELECT SCOPE_IDENTITY();";

                var parameters = new SqlParameter[]
                {
                    new SqlParameter("@StudentCode", student.StudentCode.Trim()),
                    ...
                    new SqlParameter("@Phone", student.Phone.Trim()),
                    new SqlParameter("@Email", student.Email.Trim()),
                    new SqlParameter("@Address", (object?)student.Address?.Trim() ?? DBNull.Value),
                    new SqlParameter("@ClassName", student.ClassName.Trim()),
                    new SqlParameter("@Faculty", student.Faculty.Trim())
                };
                ...
                string username = student.StudentCode.Trim().ToLower();
                string passwordHash = PasswordHelper.HashPassword("123456");

                string checkUserSql = "SELECT COUNT(1) FROM dbo.Users WHERE Username = @Username";
                var userCount = DatabaseHelper.ExecuteScalar(checkUserSql, conn, tran, new[] { new SqlParameter("@Username", username) });

                if (userCount == null || Convert.ToInt32(userCount) == 0)
                {
                    string insertUserSql = @"
                        INSERT INTO dbo.Users (Username, PasswordHash, FullName, Role, StudentID, IsActive, CreatedAt)
                        VALUES (@Username, @PasswordHash, @FullName, 'SinhVien', @StudentID, 1, GETDATE())";
        ...
        public bool UpdateStudent(Student student, out string errorMessage)
        {
            ...
            string checkSql = "SELECT COUNT(1) FROM dbo.Students WHERE StudentCode = @StudentCode AND StudentID != @StudentID";
        ...
        public bool DeleteStudent(int studentId, out string errorMessage)
        {
            ...
            string checkStaying = "SELECT COUNT(1) FROM dbo.RoomAssignments WHERE StudentID = @StudentID AND Status = 'Active'";
            var countStaying = DatabaseHelper.ExecuteScalar(checkStaying, new[] { new SqlParameter("@StudentID", studentId) });
