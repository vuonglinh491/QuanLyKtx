        ...
        private void LoadSummaryCards(int year)
        {
            try
            {
                ...
                string sqlTotal = @"
                    ...
                    FROM Payments
                    WHERE CAST(strftime('%Y', PaymentDate) AS INTEGER) = @Year AND Status = 'Paid'";

                var objTotal = DatabaseHelper.ExecuteScalar(sqlTotal, new[] { new SQLiteParameter("@Year", year) });
                ...
                string sqlRoom = @"
                    SELECT COALESCE(SUM(Amount), 0)
                    FROM Payments
                    WHERE CAST(strftime('%Y', PaymentDate) AS INTEGER) = @Year AND Status = 'Paid' AND PaymentType = 'Room'";

                var objRoom = DatabaseHelper.ExecuteScalar(sqlRoom, new[] { new SQLiteParameter("@Year", year) });
                ...
                string sqlUtil = @"
                    SELECT COALESCE(SUM(Amount), 0)
                    FROM Payments
                    WHERE CAST(strftime('%Y', PaymentDate) AS INTEGER) = @Year AND Status = 'Paid' AND PaymentType = 'ElectricityWater'";

                var objUtil = DatabaseHelper.ExecuteScalar(sqlUtil, new[] { new SQLiteParameter("@Year", year) });
                ...
                // Tỷ lệ lấp đầy
                string sqlOcc = @"
                    SELECT COALESCE(SUM(CurrentOccupancy), 0) AS Occ, COALESCE(SUM(Capacity), 0) AS Cap
                    FROM Rooms";
            ...
            try
            {
                string sql = @"
                    ...
                    SELECT 
                        ...
                        @Year AS [Năm],
                        COALESCE(SUM(CASE WHEN p.PaymentType = 'Room' THEN p.Amount ELSE 0 END), 0) AS [Tiền phòng (đ)],
                        COALESCE(SUM(CASE WHEN p.PaymentType = 'ElectricityWater' THEN p.Amount ELSE 0 END), 0) AS [Điện nước (đ)],
                        COALESCE(SUM(CASE WHEN p.PaymentType = 'Violation' THEN p.Amount ELSE 0 END), 0) AS [Phạt vi phạm (đ)],
                        COALESCE(SUM(p.Amount), 0) AS [Tổng doanh thu (đ)]
                    FROM Months m
                    LEFT JOIN Payments p ON CAST(strftime('%m', p.PaymentDate) AS INTEGER) = m.[Month] AND CAST(strftime('%Y', p.PaymentDate) AS INTEGER) = @Year AND p.Status = 'Paid'
                    GROUP BY m.[Month]
                    ORDER BY m.[Month]";

                var dt = DatabaseHelper.ExecuteQuery(sql, new[] { new SQLiteParameter("@Year", year) });
            ...
            try
            {
                string sql = @"
                    SELECT 
                        ...
                        COALESCE(SUM(r.Capacity), 0) AS [Tổng sức chứa (chỗ)],
                        COALESCE(SUM(r.CurrentOccupancy), 0) AS [Đang lưu trú (SV)],
                        COALESCE(SUM(r.Capacity - r.CurrentOccupancy), 0) AS [Số chỗ trống],
                        CASE WHEN COALESCE(SUM(r.Capacity), 0) > 0 
            ...
            try
            {
                string sql = @"
                    SELECT 
                        ...
                        COALESCE(SUM(v.FineAmount), 0) AS [Tổng tiền phạt (đ)],
                        COALESCE(SUM(CASE WHEN v.Status = 'Paid' THEN v.FineAmount ELSE 0 END), 0) AS [Đã thu (đ)],
                        COALESCE(SUM(CASE WHEN v.Status = 'Unpaid' THEN v.FineAmount ELSE 0 END), 0) AS [Chưa thu (đ)]
                    FROM Violations v
                    WHERE CAST(strftime('%Y', v.ViolationDate) AS INTEGER) = @Year
