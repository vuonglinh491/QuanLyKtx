            ...
            try
            {
                string sql = @"
                    SELECT 
                        ...
                        CASE p.PaymentType
                            WHEN 'Room' THEN N'Tiền phòng'
                            WHEN 'ElectricityWater' THEN N'Điện nước'
                            WHEN 'Violation' THEN N'Vi phạm'
                            ELSE p.PaymentType
                        END AS [Loại chi phí],
                        p.Amount AS [Số tiền (đ)],
                        CONVERT(VARCHAR(10), p.PaymentDate, 103) AS [Ngày nộp],
                        CASE p.PaymentMethod
                            WHEN 'Cash' THEN N'Tiền mặt'
                            WHEN 'Transfer' THEN N'Chuyển khoản'
                            ELSE p.PaymentMethod
                        END AS [Phương thức],
                        CASE p.Status
                            WHEN 'Paid' THEN N'Đã hoàn thành'
                            WHEN 'Pending' THEN N'Chờ xử lý'
                            WHEN 'Failed' THEN N'Thất bại'
                            ELSE p.Status
                        END AS [Trạng thái],
                ...
                if (cboFilterType.SelectedIndex == 1)
                    sql += " AND p.PaymentType = 'Room'";
                else if (cboFilterType.SelectedIndex == 2)
                    sql += " AND p.PaymentType = 'ElectricityWater'";
                else if (cboFilterType.SelectedIndex == 3)
                    sql += " AND p.PaymentType = 'Violation'";
