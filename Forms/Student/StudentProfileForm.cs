...
namespace QuanLyKtx.Forms.Student
{
    public partial class StudentProfileForm : Form
    {
        ...
        private void LoadStudentData()
        {
            ...
            {
                string sql = @"
            SELECT StudentCode, FullName, DateOfBirth, Gender, Phone, Email, Address, ClassName, Faculty
            FROM Students
        ...
        private void btnSaveContact_Click(object? sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    new SQLiteParameter("@StudentID", Session.StudentID.Value),
        ...
        private void btnChangePassword_Click(object? sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string checkSql = "SELECT PasswordHash FROM Users WHERE UserID = @UserID";
                var objHash = DatabaseHelper.ExecuteScalar(checkSql, new[] { new SQLiteParameter("@UserID", Session.UserID) });
                ...
                string updateSql = "UPDATE Users SET PasswordHash = @PasswordHash WHERE UserID = @UserID";
                int affected = DatabaseHelper.ExecuteNonQuery(updateSql, new[]
                {
                    new SQLiteParameter("@PasswordHash", newHash),
                    new SQLiteParameter("@UserID", Session.UserID)
                });
